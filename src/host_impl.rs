...
impl Processor {
    ...
    pub fn setup_processing(&self,
                            mode : ProcessMode,
                            sample_size : SampleSize,
                            max_block_size : usize,
                            sample_rate : f64) -> Result<(), tresult> {
...
impl Editor {
    ...
    pub fn set_state(&self, stream : &BStream) -> Result<(), tresult> {
        let e = unsafe {
            (*self.0).setState(stream.0.as_raw())
        };
    ...
    pub fn get_state(&self, stream : &BStream) -> Result<(), tresult> {
        let e = unsafe {
            (*self.0).getState(stream.0.as_raw())
        };
    ...
    pub fn get_param_string_by_value(&self, id : ParamID, value : f64)-> Result<String, tresult> {
        let mut s = [0; 128];
        let e = unsafe {
            self.0.getParamStringByValue(id, value, s.as_mut_ptr())
        };
        if e != 0 { Err(e) }
        else { unsafe {  Ok(U16CStr::from_ptr_str(transmute(s.as_ptr())).to_string_lossy() ) } }
    }
...
impl PlugView {
    ...
    pub fn attached(&self, parent : *mut c_void) -> Result<(), tresult>{ unsafe {
    ...
    pub fn get_size(&self) -> Result<ViewRect,tresult> {
        ...
        let e = unsafe {
            self.0.getSize(&mut rect as *mut _)
        };
        if e != 0 { Err(e) }
        else { Ok(rect) }
    }
