...
fn open(controller: Editor) {
    ...
    if let Some(display) = window.get_xlib_display() {
        let atom =
            unsafe { XInternAtom(display as *mut _, transmute("_XEMBED_INFO\0".as_ptr()), 1) };
        if atom == 0 {
            panic!("XEMBED_INFO does not exist");
        }
...
fn main() {
    ...
    lib_path.push(opt.path.as_path().file_stem().unwrap().to_str().unwrap());
    lib_path.set_extension("so");

    let lib = ll::Library::new(lib_path.as_path()).expect("no such library");
    ...
    for inst in instances.iter() {
        ...
        if let Ok(processor) = inst.0.cast::<IAudioProcessor>() {
            ...
            if proc.supports32() {
                println!("\t32 bit audio supported");
            }
            if proc.supports64() {
                println!("\t64 bit audio supported");
            }
            if let Ok(_) =
                proc.setup_processing(ProcessMode::Offline, SampleSize::F32, 512, 48000.0)
            {
